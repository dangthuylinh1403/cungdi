...
const getTierConfig = (tier: MembershipTier = 'standard') => {
    switch (tier) {
        case 'silver': return { label: 'Bạc', icon: Medal, color: 'text-slate-400', discountVal: 10 };
        case 'gold': return { label: 'Vàng', icon: Trophy, color: 'text-amber-400', discountVal: 20 };
        case 'diamond': return { label: 'Kim Cương', icon: Gem, color: 'text-cyan-400', discountVal: 30 };
        case 'family': return { label: 'Gia Đình', icon: Heart, color: 'text-rose-400', discountVal: 80 };
        default: return { label: 'Thường', icon: Award, color: 'text-slate-500', discountVal: 0 };
    }
};
...
const RoadAnimation = () => {
  ...
  return (
    <div className="flex-1 max-w-md mx-3 md:mx-8">
...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab, notifications, clearNotification, profile, profileLoading, onLoginClick, onProfileClick, pendingOrderCount = 0, activeTripsCount = 0, activeBookingsCount = 0 }) => {
  ...
  const navItems = [
    ...
    { id: 'my-trips', label: 'Chuyến xe', icon: Car }, // Thay thế Bookings
    { id: 'my-requests', label: 'Yêu cầu', icon: Ticket }, // Thay thế Bookings
  ];
      ...
      {profile && (
        <div className="h-9 shrink-0 bg-emerald-100/80 backdrop-blur-sm border-b border-emerald-200 text-slate-700 flex items-center justify-between px-4 text-xs font-bold z-50">
          <div className="flex items-center gap-2">
            {/* Tier Badge */}
            <div className="flex items-center gap-1.5 bg-white px-2 py-1 rounded-full border border-slate-200 shadow-sm" title={`Cấp độ thành viên: ${tierConfig.label}`}>
              <TierIcon size={12} className={tierConfig.color} />
              <span className="text-slate-700 text-[10px] font-bold">{tierConfig.label}</span>
            </div>
            
            {/* Discount Badge */}
            {tierConfig.discountVal > 0 && (
              <div className="flex items-center gap-1 bg-rose-100 px-2 py-1 rounded-full border border-rose-200">
                <span className="font-black text-rose-600 text-[10px]">-{tierConfig.discountVal}%</span>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            {activeTripsCount > 0 && (
              <div className="flex items-center gap-1.5 bg-emerald-100/70 text-emerald-800 px-3 py-1.5 rounded-full border border-emerald-200/80">
                <Car size={12} className="text-emerald-600" />
                <span className="font-bold text-[10px]">{activeTripsCount} Đang chạy</span>
              </div>
            )}
            {activeBookingsCount > 0 && (
              <div className="flex items-center gap-1.5 bg-orange-100/70 text-orange-800 px-3 py-1.5 rounded-full border border-orange-200/80">
                <Zap size={12} className="text-orange-600" />
                <span className="font-bold text-[10px]">{activeBookingsCount} Yêu cầu</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden flex-col xl:flex-row">
        ...
        <aside className="hidden xl:flex flex-col w-72 bg-gradient-to-b from-emerald-50/80 to-indigo-50/60 border-r border-slate-100 p-8 shrink-0">
          ...
          <nav className="flex-1 space-y-1.5 overflow-y-auto custom-scrollbar pr-2">
            ...
            {/* Moved Post Trip here for everyone */}
            <button
              type="button"
